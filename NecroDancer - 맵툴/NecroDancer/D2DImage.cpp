...
HRESULT D2DImage::D2DSetting(LPCWSTR fileName)
{
	...
	hr = _imageInfo->pWICImagingFactory->CreateDecoderFromFilename(		//디코더 생성
		fileName,
		0,
		GENERIC_READ,					//읽기전용
	...
	hr = _imageInfo->pWICDecoder->GetFrame(0, &_imageInfo->pWICFrameDecoder);		//0번째 프레임을 가져옴

	if (FAILED(hr))
	{
		_imageInfo->pWICDecoder->Release();
		return hr;
	}

	hr = _imageInfo->pWICImagingFactory->CreateFormatConverter(&_imageInfo->pWICConverter);

	if (FAILED(hr))
	{
		_imageInfo->pWICConverter->Release();
		return hr;
	}

	hr = _imageInfo->pWICConverter->Initialize(
		_imageInfo->pWICFrameDecoder,
	...
	if (FAILED(hr))
	{
		_imageInfo->pWICConverter->Release();
		_imageInfo->pWICDecoder->Release();
		_imageInfo->pWICFrameDecoder->Release();
		return hr;
	}

	//hr = D2DMANAGER->GetD2DRenderTarget()-(_imageInfo->pWICConverter, nullptr, &_imageInfo->pBitmap);
	hr = D2DMANAGER->GetD2DRenderTarget()->CreateBitmapFromWicBitmap(_imageInfo->pWICConverter, nullptr, &_imageInfo->pBitmap);
...
void D2DImage::Render(float alpha)
{
	...
	D2D1_RECT_F dxArea1 = D2D1::RectF(0, 0, _imageInfo->width, _imageInfo->height);
...
void D2DImage::Render(float destX, float destY,float width, float height, float alpha)
{
	...
	D2D1_RECT_F dxArea1 = D2D1::RectF(destX, destY, destX + width, destY + height);
	D2D1_RECT_F dxArea2 = D2D1::RectF(0, 0, _imageInfo->width, _imageInfo->height);
	D2DMANAGER->GetD2DRenderTarget()->DrawBitmap(_imageInfo->pBitmap, dxArea1, alpha, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, dxArea2);
}
...
void D2DImage::Render(float destX, float destY, float sourX, float sourY, float sourWidth, float sourHeight, float alpha)
{
	...
	D2D1_RECT_F dxArea1 = D2D1::RectF(destX, destY, destX + sourWidth, destY + sourHeight);
	D2D1_RECT_F dxArea2 = D2D1::RectF(sourX, sourY, sourX + sourWidth, sourY + sourHeight);

	D2DMANAGER->GetD2DRenderTarget()->DrawBitmap(_imageInfo->pBitmap, dxArea1, alpha, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, dxArea2);
}

void D2DImage::RenderTest(float destX, float destY, float alpha)	//이미지 반전 테스트
{
	if (_imageInfo->pBitmap == NULL) return;

	destX = destX * -1 - _imageInfo->width;		//
...
void D2DImage::FlipRender(float destX, float destY, float width, float height, float alpha )
{
	...
	D2D1_RECT_F dxArea1 = D2D1::RectF(destX, destY, destX + width, destY + height);
	D2D1_RECT_F dxArea2 = D2D1::RectF(0, 0, _imageInfo->width, _imageInfo->height);
	D2DMANAGER->GetD2DRenderTarget()->SetTransform(D2D1::Matrix3x2F::Matrix3x2F(-1, 0, 0, 1, 0, 0));		//좌우반전을 위한 렌더타겟 매트릭스 변경
	D2DMANAGER->GetD2DRenderTarget()->DrawBitmap(_imageInfo->pBitmap, dxArea1, alpha, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, dxArea2);
	D2DMANAGER->GetD2DRenderTarget()->SetTransform(D2D1::Matrix3x2F::Identity());							//변경했던 렌더타겟 매트릭스 초기화
}
void D2DImage::FlipFrameRender(float destX, float destY, float alpha )
{
	...
	destX = destX * -1 - _imageInfo->frameWidth;
