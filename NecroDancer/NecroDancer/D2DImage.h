...
public:
	...
	//좌우 반전 애니메이션
	void FlipAniRender(float destX, float destY, Animation* ani, float alpha = 1.0f);
	...
	void FlipRender(float destX, float destY, float sourX, float soury, float sourWidth, float soourHeight, float alpha = 1.0f);
	...
	void ReverseFrameRender(float destX, float destY, int frameCount, float alpha = 1.0f);


	void UIRender(float destX, float destY, float alpha = 1.0f);
	void UIFrameRender(float destX, float destY, int currentFrameX, int currentFrameY, float alpha = 1.0f);
	...
	int GetMaxFrmame() { return (_imageInfo->maxFrameX+1) * (_imageInfo->maxFrameY+1) - 1; }
};
